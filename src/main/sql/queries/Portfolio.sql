...
select * from portfolio_cashflow where memberid in (1071) order by portfolioid, date desc;
select * from portfolio_value_history a where memberid in (1071) and  date >= '2023-02-15' order by memberid, date desc;
...
and b.compositeid in (1,2,3,4,7) -- change to 1: for INTRO strategy 2: FOCUS-FIVE
...
select * from mosl_transaction where /*quantity < 0 and*/ date >= '2023-02-22' and script_name not in ('MOSL_CASH', 'LIQUIDBEES') AND moslcode not in ('-H20404', '-1') and is_processed != '-Y' order by date, moslcode;
select moslcode, date, script_name, sell_buy, sum(quantity), sum(brokerage) from mosl_transaction where date >= '2023-02-22' and script_name not in ('MOSL_CASH', 'LIQUIDBEES') group by moslcode, date, script_name, sell_buy order by moslcode, date desc, script_name;
...
select * from portfolio_historical_holdings a where a.memberid in (1) and sell_date > '2022-12-01' order by sell_date desc, ticker;
...
select * from portfolio_holdings a where a. memberid = 1000 order by a.memberid, a.portfolioid, a.asset_classid, a.ticker, a.buy_date;
