...
select * from mosl_transaction where /*quantity < 0 and*/ date >= '2024-03-01' and script_name not in ('MOSL_CASH', 'LIQUIDBEES') AND moslcode not in ('-H20404', '-1') and is_processed != '-Y' order by date, moslcode;
select moslcode, date, script_name, sell_buy, sum(quantity), sum(brokerage), sum(net_amount) from mosl_transaction where date >= '2024-03-01' and moslcode in ('H1') and script_name not in ('MOSL_CASH', 'LIQUIDBEES') group by moslcode, date, script_name, sell_buy order by moslcode, date desc, script_name;
