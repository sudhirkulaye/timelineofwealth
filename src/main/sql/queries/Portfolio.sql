...
select * from portfolio_cashflow where memberid in (1018) order by portfolioid, date desc;
select * from portfolio_value_history a where memberid in (1000, 1070) and  date >= '2022-08-01' order by memberid, date desc;
...
select * from mosl_transaction where /*quantity < 0 and*/ date >= '2022-12-01' and script_name not in ('MOSL_CASH', 'LIQUIDBEES') AND moslcode not in ('-H20404', '-1') and is_processed != '-Y' order by date, moslcode;
