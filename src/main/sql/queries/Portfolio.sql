...
select * from mosl_transaction where /*quantity < 0 and*/ date >= '2025-09-09' and script_name not in ('MOSL_CASH', 'LIQUIDBEES') AND moslcode not in ('-H20404', '-1') and is_processed != '-Y' order by date, moslcode;
...
select * from portfolio_historical_holdings a where a.memberid in (1000,1011) and sell_date >= '2025-01-01' order by sell_date desc, ticker;
...
select * from portfolio_cashflow a where a. memberid = 1000 and portfolioid = 1 order by date desc;
select * from portfolio_holdings a where a. memberid in (1000) order by a.memberid, a.portfolioid, a.asset_classid, a.ticker, a.buy_date;
