...
select * from mosl_transaction where /*quantity < 0 and*/ date >= '2026-01-30' and script_name not in ('MOSL_CASH', 'LIQUIDBEES') AND moslcode not in ('-H20404', '-1') and is_processed != '-Y' order by date, moslcode;
...
select * from portfolio_historical_holdings a where a.memberid in (1) and sell_date >= '2025-02-24' order by sell_date desc, ticker;
