...
select round(sum(market_value)), count(market_value) from wealth_details a  where a.memberid in (1007, 1015, 1058) and short_name like '%-%Dir%-%' order by memberid, name;
select round(sum(market_value)), count(market_value) from wealth_details a  where a.memberid in (1007, 1015, 1058) and short_name like '%-%Reg%-%' order by memberid, name;
SELECT count(1) from sip a where a.scheme_name like '%-%Reg%-%' and a.memberid in (1007, 1015, 1058);
